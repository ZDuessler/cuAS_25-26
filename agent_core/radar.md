...
This plugin lets C3Mission read values from the radar relay server. It spins up a WebSocket client on a separate thread to connect to the relay.
...
`filterTracks()` takes five arguments, all of which are optional. You can choose to filter by age, RCS, category, certainty, or ID, or any combination of the five. Age, RCS, and certainty default to zero if left unspecified, meaning they won't filter out any tracks; category defaults to a list of all possible categories; and ID defaults to "NUL", which never occurs as a track ID.
...
Methods which return multiple tracks (`getAll()`, `filterTracks()`, and `excludeByLLA()`) produce a list where each member is a track, sorted by greatest RCS to smallest.
...
```python
{
    ...
    "isStationary": bool, # whether or not object is stationary
    "category": str, # track category (bird, droneplane, dronerotor, airplane, helicopter, other)
    "catProb": { # probability by possible category, given in percentage (NOT decimal out of 1.00)
        "other": float,
        "bird": float,
        "dronerotor": float,
        "droneplane": float,
...
Using `Track.raw` gives you access to this dictionary. Since dictionaries are faster than classes, this library uses the dictionary for all filtering operations internally, but returns the corresponding `Track()` object.
...
The plugin expects over the WebSocket connection JSON strings encoding the following structure:

```javascript
[
    [{ // array of radar tracks
        id: String, // SkyDome-assigned ID
        rcs: Number, // RADAR cross-section
        start: Number, // UNIX time of track's appearance
        lla: [Number, Number, Number], // lat, long, rel alt
        category: String, // RADAR's classification of track
        catProb: { // certainties of all possible classifications
            bird: Number,
            droneplane: Number,
            dronerotor: Number,
            airplane: Number,
        ...
        heading: Number
    }, { /* additional radar tracks */ }],

    Number, // message type (0 for data)
    Number, // UNIX time from Date.now()

    { // catalog mapping ID string to array index
        "XXX": String
    }
]
```

If there was an error in polling the RADAR, the message is formatted as this instead:

```javascript
[
    String, // error message
    Number, // message type (-1 for error)
    Number, // UNIX time from Date.now()
]
```
